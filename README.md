...
**状态：P3 用量核算开发中**（P1 透明直通、P2 审计模式均已在 PVE 9.2.3 测试集群验证）。P3 新增：只读服务账号按池配置实时核算各用户用量（`cores`/`memory`/`disk.<storage>` 含 `unused[n]`/`instances`），配额存储 `quotas.yaml`（默认拒绝、校验、热加载），经管理端点 `/usage` 暴露；**尚不拦截**（P4 才拦截）。见 [Docs / phases.md](https://github.com/WilliamLi0623/ProxmoxUserQuota-Docs/blob/main/phases.md)。
